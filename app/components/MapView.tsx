...
// --- React-Leaflet: import solo client ---
...
export default function MapView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Icone default da CDN (evita 404 in prod)
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      if (!r.ok) throw new Error("bad pwd");
  ...
  // Deterrenti anti-inspect (si disattivano con ?debug=1)
  useEffect(() => {
    ...
    <section className="relative h-full w-full">
      {leafletReady && (
        ...
        >
          ...
          {/* Rotta prevista: LHR -> KIN (tratteggiata ciano) */}
          <Polyline positions={straight} pathOptions={{ color: "#22d3ee", weight: 3, dashArray: "6 8" }} />

          {/* Deviazione: LHR -> Bermuda -> Ceprano (fucsia) */}
          <Polyline positions={diverted} pathOptions={{ color: "#f472b6", weight: 4 }} />

          {/* Triangolo delle Bermuda (giallo) */}
          <Polyline
            positions={[TRIANGLE.miami, TRIANGLE.sanJuan, TRIANGLE.bermuda, TRIANGLE.miami]}
            pathOptions={{ color: "#fbbf24", weight: 2, dashArray: "6 8", opacity: 0.7 }}
          />

          {/* Marker */}
          <Marker position={LHR}>
            <Popup>Partenza: London Heathrow (LHR)</Popup>
          </Marker>

          <Marker position={KIN}>
            <Popup>Destinazione prevista: Kingston (KIN), Giamaica</Popup>
          </Marker>
